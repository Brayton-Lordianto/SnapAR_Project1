...
script.createBones = function(prefix, isThumb, offset) {
    for (var i = 0; i < 3; i++) {
        ...
        var jointCollider = new global.fingerCollider(script.colliderObject, script.hand.getJoint(current), script.getSceneObject());
        ...
        var midCollider = new global.fingerCollider(script.colliderObject, script.hand.getJoint(current), script.getSceneObject(), script.hand.getJoint(next));
    ...
    var bone = new global.palmBone(script.colliderObject, script.hand, script.hand.getJoint(baseJoint), offset, script.getSceneObject());
    script.palmBones.push(bone);
    
    for (var j = 0; j < bone.objects.length; j++) {
        script.colliderObjects.push(bone.objects[j]);
